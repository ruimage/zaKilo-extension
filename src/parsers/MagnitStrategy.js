...
export class MagnitStrategy extends ParserStrategy {
  ...
  _parseQuantity(text) {
    ...
    switch (unit) {
      case 'г': case 'гр':
        unitLabel = '1 кг'; multiplier = value / 1000; break;
      case 'кг':
        unitLabel = '1 кг'; multiplier = value; break;
      case 'мл':
        unitLabel = '1 л';  multiplier = value / 1000; break;
      case 'л':
        unitLabel = '1 л';  multiplier = value; break;
      case 'шт': case 'шт.':
        unitLabel = '1 шт'; multiplier = value; break;
      default:
        unitLabel = `1 ${unit}`; multiplier = value;
    }
    return { unitLabel, multiplier };
  }

  _renderUnitPrice(cardEl, unitPrice, unitLabel) {
    const priceContainer = cardEl.querySelector(
      '.unit-catalog-product-preview-prices'
    );
    if (!priceContainer) throw new Error('Price container not found');

    priceContainer.querySelectorAll(this.selectors.unitPrice)
      .forEach(el => el.remove());

    const displayValue =
      unitPrice < 20
        ? unitPrice.toFixed(2)
        : Math.ceil(unitPrice).toString();
